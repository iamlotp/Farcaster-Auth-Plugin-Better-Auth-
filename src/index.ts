import type { BetterAuthClientPlugin, BetterAuthPlugin, User as BetterAuthUser } from "better-auth";
...
config();
const HTTP_HUB_DOMAIN = process.env.FARCASTER_HTTP_HUB || 'hub.pinata.cloud'
const AUTH_URL = process.env.BETTER_AUTH_URL || process.env.APP_URL
        ...
        endpoints: {
            ...
            verifyFarcasterSignIn: createAuthEndpoint("/farcaster/verify", { method: "POST" }, async (ctx) => {
                ...
                try {
                    ...
                    const farcasterData = await getUserDataByFid(fid, HTTP_HUB_DOMAIN);
                    ...
                    const farcasterUserDb = await ctx.context.adapter.findOne({
                        model: "farcasterUser",
                        where: [
                            { field: "fid", value: fid.toString() },
                        ],
                    ...
                    if (!user) {
                        ...
                        if (
                            (farcasterData.PFP !== farcasterUserDb.pfp) ||
                            (farcasterData.USERNAME !== farcasterUserDb.username) ||
                            (farcasterData.DISPLAY !== farcasterUserDb.displayName) ||
                            (farcasterData.TWITTER !== farcasterUserDb.twitterUsername)
                        ) {
